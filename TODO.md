...
**Risolti in giornata (segue):**
- [x] **A3 — incertezze statistiche in cap6** (FATTO 2026-07-03, tutto da cache esistenti — zero re-run; script one-shot in scratchpad, valori verificati identici alle tabelle). (1) `slice_fit.fit_layers` ora ritorna `slope_err` (σ_m = √(ss_res/(N−1)/Σn²)); notebook cella F lo salva in `STRATI_SLICE_RESULTS`. (2) cap6: tab:nastro_slope m = 240,2±0,9 / 280,8±1,5 / 316,8±1,7 °/strato (+ prosa); k nastro = (1,492±0,010)·10⁵ °·nm/strato; k Drude zucchero = (−1,390±0,056)·10⁶ °·nm² (k_err era già in `dispersion.fit_inverse_lambda`, mai riportato). (3) sec:discussione: paragrafo incertezza esteso — errori di fit = SOLO statistici (riproducibilità interna), accuratezza assoluta dominata dalle sistematiche. (4) Cluster vincente: decisione utente = **frase in prosa, niente colonna** (coerente con rigetto σ_cluster 2026-05-21): MAD circolare ≤1,2° (λ/4), <0,6° (λ/2), <0,5° (zucchero AoLP), etichettata come omogeneità della regione, non incertezza. SE mediana (~0,05°) NON usato: fuorviante (pixel correlati). Compila pulito, 0 undefined. **BOZZA, DA RIVEDERE.**
