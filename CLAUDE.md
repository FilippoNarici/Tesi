...
* **Ambiguità di wrap modulo-360°** per campioni ad alto ritardo (nastro multistrato): richiede interpretazione umana; vedi `chapters/cap6_risultati.tex`. **La `lambdamezzi_50deg` NON è multi-ordine (falsificato 2026-06-12)**: la ritenzione di polarizzazione intra-banda (`eq:band_retention`, spettri reali, `python/verify_joint_order.py`) esclude ordini ≥2 (DoP_G prevista ≤0.78 vs 0.845 misurata, robusto al modello di banda). La δ discendente (176.9/156.8/134.5, s3>0 nel disco) è **dispersione di ritardanza inversa reale**: ritardatore wideband/acromatico a ordine zero (Re466/Re536=0.746, ramo complementare 0.96, cellophane-ideale 0.87; famiglia cellophane/film reverse, cite `OrtizGutierrez2001Cellophane`+`Uchiyama2012Copolycarbonate`). λ/4 = ordine zero a dispersione normale. Ipotesi costruttiva: film sottile (~decine µm) laminato fra vetri (i mm visibili = vetro). cap6 λ/2 riscritta 2026-06-12 col percorso multi-ordine→falsificazione→reverse; registro ipotesi completo in `python/WAVEPLATE_INVESTIGATION.md`. Nota storica valida: invert_angles e swap S3 si compensano sulla δ → δ INVARIATA per strati/barra/lambdaquarti/righello; θ cambia ovunque (~50→~34/37). Dettaglio in memory `project_waveplate_multiorder.md`.
* **Correzione S3 ora DATA-DRIVEN (2026-06-11):** δ_a(λ) dell'analizzatore λ/4 **misurato** direttamente (`config.MEASURED_S3_RETARDANCE_DEG` = 88.4/111.0/130.8 R/G/B, `USE_MEASURED_S3_RETARDANCE=True`), non più assunto dal modello quarzo Ghosh. Misura non circolare + depol-unbiased + material-free, sfrutta λ/4 sample==analizzatore (processo completo in **`python/S3_CALIBRATION.md`**; vedi anche memory `project_s3_datadriven_calibration.md` + cap6 `sec:calibrazione_s3`). Quarzo (`quartz_birefringence`, `waveplate_retardance`) resta SOLO come fallback nel codice: **rimosso da tutta la tesi (2026-06-12)** — a 3 bande larghe quarzo/PMMA/PC indistinguibili (`python/verify_polymer_riscontro.py`), riferimento in tesi = sola legge geometrica δ0·λ0/λ (scarti −2.6/+4.7/+8.5 R/G/B = dispersione normale, material-free). **Paradigma lamine (2026-06-12):** nessun materiale/ordine assunto a priori (etichetta = solo ritardanza nominale @633 nm); caratterizzazione a posteriori in cap6 (compatibile singolo ordine per λ/4, multi-ordine per λ/2); "zero-order" compare solo nella definizione teorica di cap2. Helper `stokes.analyzer_retardance(ch_idx, λ)`.
