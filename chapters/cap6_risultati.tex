...
L'elemento dirimente è lo spessore, dell'ordine del millimetro, a fronte dei circa $\SI{17.6}{\micro\meter}$ di una lamina $\lambda/4$ zero-order in quarzo a $\SI{633}{\nano\meter}$. Si formula pertanto l'ipotesi che la lamina $\lambda/2$ sia un ritardatore multi-ordine, nel quale la ritardanza totale $\delta_\mathrm{tot}(\lambda) = 360^\circ\,\Delta n(\lambda)\,d/\lambda$ attraversa molti multipli di $360^\circ$ e la misura ne restituisce il solo residuo $\delta_\mathrm{tot}\bmod 360^\circ$. Per un simile ritardatore la dispersione del valore avvolto non segue più la legge zero-order, e il riferimento adottato in prima battuta perde validità. Un fit del modello multi-ordine ai tre valori misurati individua uno spessore efficace di circa $\SI{0.38}{\milli\meter}$ (ordine $\sim 5$) con scarto quadratico medio di $\SI{7}{\degree}$, contro i $\SI{18}{\degree}$ del modello zero-order; lo stesso fit applicato alla lamina $\lambda/4$ restituisce $\sim\SI{17.6}{\micro\meter}$, coincidente con l'ordine zero. L'ambiguità di avvolgimento impedisce di fissare univocamente ordine e spessore, ma l'ipotesi multi-ordine descrive i dati in modo nettamente migliore (\cref{fig:hw_multiorder}).

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.6\textwidth]{Images/generated/lambdamezzi_50deg/delta_multiorder_fit.pdf}
    \caption{Fit multi-ordine della lamina $\lambda/2$: la curva tratteggiata è la ritardanza totale avvolta $\delta_\mathrm{tot}(\lambda) = 360^\circ\,\Delta n(\lambda)\,d/\lambda \bmod 360^\circ$ per lo spessore efficace ottimale ($d \approx \SI{0.38}{\milli\meter}$, ordine $\sim 5$); i punti colorati sono le stime $\tilde\delta_{\mathrm{win}}$ per canale, il diamante nero il punto di design (non incluso nel fit). Il modello multi-ordine segue l'andamento avvolto dei punti misurati, che la legge zero-order non riproduce.}
    \label{fig:hw_multiorder}
\end{figure}

Si conclude, in via di ipotesi, che la lamina $\lambda/2$ del kit sia un ritardatore ad alto ordine: la pipeline ne misura correttamente la ritardanza avvolta, e l'apparente disaccordo con il modello zero-order riflette l'ambiguità di avvolgimento modulo $360^\circ$ propria dei campioni ad alto ritardo (\cref{subsec:modello_retardance}), più che un limite di accuratezza del polarimetro.
...
Il canale rosso costituisce il riferimento. La slice individua i nove plateau attesi (\cref{fig:nastro_slice_r}), con mediane che avvolgono da $\SI{247.6}{\degree}$ del primo strato fino a riavvolgersi più volte; l'istogramma in \cref{fig:nastro_hist_r} mostra la corrispondente struttura discreta dei valori avvolti sul campione. Dopo l'unwrap, i punti $(n, \delta)$ giacciono su una retta con pendenza $m_R = \SI{240.2}{\degree}$/strato, coefficiente di determinazione $R^2 = 0{,}9994$ e scarto quadratico medio di $\SI{7.7}{\degree}$.
...
Un'eccezione merita di essere segnalata. Confrontando i valori srotolati delle regioni omologhe a tre strati, l'asimmetria fra lato sinistro e destro è contenuta sul rosso ($\SI{735.1}{\degree}$ contro $\SI{714.9}{\degree}$, differenza $\SI{20.2}{\degree}$) e sul verde ($\SI{840.2}{\degree}$ contro $\SI{825.6}{\degree}$, $\SI{14.6}{\degree}$), ma sale a $\SI{46.7}{\degree}$ sul blu ($\SI{976.9}{\degree}$ contro $\SI{930.2}{\degree}$). L'anomalia è specifica del singolo strato (il terzo) e del singolo canale (il blu): non si manifesta negli altri strati né sugli altri canali, ed è perciò incompatibile con un'asimmetria fisica generica del campione, che si propagherebbe in modo proporzionale alla pendenza su tutti gli strati e tutti i canali. Le verifiche condotte hanno escluso un difetto di allineamento dei frame, una depolarizzazione locale e un'asimmetria della distribuzione dei vettori di Stokes nelle due regioni. La causa resta a oggi non spiegata; fra le ipotesi residue compatibili con la specificità in lunghezza d'onda figurano una registrazione cromatica imperfetta del canale blu, che mapperebbe l'ancora della slice su posizioni fisiche leggermente diverse, e un'interferenza sottile selettiva in lunghezza d'onda negli strati di adesivo. La questione è documentata come aperta e non incide sulla determinazione della pendenza media, ricavata dal fit complessivo sui nove plateau.
