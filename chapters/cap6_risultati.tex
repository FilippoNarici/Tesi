...
La ricostruzione di $S_3$ (\cref{eq:s3_formula}) contiene un unico parametro libero, il ritardo $\delta_a(\lambda)$ della lamina $\lambda/4$ impiegata come analizzatore, attraverso il fattore di correzione cromatica $1/\sin\delta_a(\lambda)$. Poiché ogni mappa di ritardanza e ogni mappa dell'asse veloce dei campioni birifrangenti dipendono da $S_3$, la scelta di $\delta_a(\lambda)$ condiziona l'intero capitolo. Adottarne un valore da un modello di materiale, come la dispersione del quarzo cristallino, lascerebbe i risultati appesi a un'assunzione non verificata, sia sul materiale della lamina sia sul suo ordine. Si è perciò preferito \emph{misurare} $\delta_a(\lambda)$ direttamente dai dati, prima di applicare la pipeline ai campioni, e relegare il modello di quarzo al ruolo di riscontro indipendente.
...
Resta da rendere la stima immune da due effetti che, se trascurati, la distorcerebbero. Il primo è la depolarizzazione introdotta dalla lamina, che riduce il grado di polarizzazione totale dal valore prossimo all'unità dello sfondo a circa $0{,}9$ nel rosso e $0{,}7$ nel blu: normalizzare sul livello dello sfondo, come fa il canale coseno, spinge artificiosamente la ritardanza verso $90^\circ$. Il secondo è l'ellitticità residua del fascio incidente, già rimossa nella pipeline dalla rotazione di Poincaré attorno all'asse $S_2$ (\cref{sec:ellitticita}). Entrambi si neutralizzano lavorando con i \emph{rapporti} fra le componenti del vettore di Stokes, ovvero con la sua direzione: sotto depolarizzazione scalare il fattore di depolarizzazione $p$ moltiplica tutte le componenti e si elide nei rapporti, lasciando inalterata la direzione dell'ellisse e quindi la ritardanza. La direzione del vettore, ricostruita sui pixel della lamina dopo gli allineamenti, fornisce così una stima di $\delta_a$ indipendente dalla normalizzazione di sfondo.
...
I valori così ottenuti sono riportati nella \cref{tab:s3_calibrazione}. La bontà interna della procedura è confermata da due verifiche che non sono state imposte ma emergono dalla soluzione: il fattore di depolarizzazione $p$ ricostruito dall'inversione riproduce il grado di polarizzazione totale misurato indipendentemente sui tre canali, decrescente verso il blu, e l'orientazione $\theta$ risulta uniforme fra i canali, come deve essere per una lamina spazialmente omogenea osservata a un'unica orientazione meccanica. Il modello di quarzo cristallino, valutato con la dispersione di Ghosh (\cref{eq:dispersione-qwp}) e mai usato nell'inversione, fornisce $90{,}9^\circ$, $107{,}5^\circ$ e $126{,}9^\circ$ sui tre canali: concorda con la ritardanza misurata entro pochi gradi senza esserne il presupposto, come mostra la \cref{fig:s3_calibration}. La compatibilità della lamina con un singolo ordine ne risulta confermata a posteriori, e il modello di quarzo legittimato come parametrizzazione di comodo di una curva che i dati determinano da soli.

\begin{table}[H]
    \centering
    \caption{Ritardanza $\delta_a$ della lamina $\lambda/4$ analizzatore, misurata direttamente dal dataset \texttt{lambdaquarti} in modo non circolare, depol-unbiased e indipendente dal materiale (vedi testo), e fattore di correzione cromatica $1/\sin\delta_a$ adottato nella ricostruzione di $S_3$. L'ultima colonna riporta, come riscontro indipendente, il ritardo previsto dal modello di quarzo cristallino (Sellmeier di Ghosh), che non interviene nella misura. L'accordo entro pochi gradi su tutti i canali rende la correzione data-driven e non vincolata all'ipotesi sul materiale. Al canale rosso $\delta_a \approx 90^\circ$ rende la correzione prossima all'unità e quindi insensibile a ogni scelta di modello.}
    \label{tab:s3_calibrazione}
    \begin{tabular}{lcccc}
        \hline
        \textbf{Canale} & $\lambda_\mathrm{eff}$ [\si{\nano\meter}] & $\delta_a$ misurato [\si{\degree}] & $1/\sin\delta_a$ & $\delta_a$ quarzo [\si{\degree}] \\
        \hline
        Rosso & 626 & $88{,}4$  & $1{,}000$ & $90{,}9$  \\
        Verde & 536 & $111{,}0$ & $1{,}071$ & $107{,}5$ \\
        Blu   & 466 & $130{,}8$ & $1{,}321$ & $126{,}9$ \\
        \hline
    \end{tabular}
\end{table}
...
La lamina quarto d'onda zero-order ($\lambda_0 = \SI{633}{\nano\meter}$), orientata a circa \SI{50}{\degree}, è stata analizzata sui tre canali colore con la stessa procedura impiegata per la lamina $\lambda/2$. Questo campione ha un ruolo speciale: è dello stesso tipo della lamina $\lambda/4$ impiegata come analizzatore di $S_3$, ed è dal suo dataset che si ricava la calibrazione non circolare della correzione cromatica di $S_3$ (\cref{sec:calibrazione_s3}). Analizzata qui come campione attraverso la pipeline completa, con la correzione già calibrata, ne riproduce per costruzione la ritardanza e ne documenta l'uniformità spaziale; il controllo di accuratezza genuino e indipendente dal materiale, ovvero il confronto fra ritardanza misurata e modello di quarzo senza assumerlo, è quello discusso nella sezione di calibrazione.
...
La stima quantitativa della ritardanza segue la stessa pipeline di clustering della \cref{sec:umap_clustering}: il cluster vincente HDBSCAN sull'embedding UMAP individua i pixel del disco della lamina e ne fornisce la mediana $\tilde\delta_{\mathrm{win}}$. Le tre stime per-canale, ottenute processando la lamina come campione con la correzione di $S_3$ calibrata, sono raccolte nella \cref{tab:qw_ritardanza} e confrontate con il modello di quarzo. Esse coincidono per costruzione con la calibrazione (\cref{tab:s3_calibrazione}) a meno di una lieve differenza, più marcata al blu, dovuta al bias di depolarizzazione della lettura combinata coseno-seno della ritardanza, che tende a riavvicinare il valore a $90^\circ$. Le stime sono fittate con la legge di dispersione $\delta(\lambda) = k/\lambda$ di una lamina zero-order; il punto di design $(\lambda_0 = \SI{633}{\nano\meter},\,\delta_0 = 90^\circ)$ è riportato come riferimento ma non incluso nel fit, condotto sui soli tre punti misurati. Il risultato è in \cref{fig:qw_delta_fit}.

\begin{table}[H]
    \centering
    \caption{Ritardanza della lamina $\lambda/4$ analizzata \emph{come campione} (zero-order, $\lambda_0 = \SI{633}{\nano\meter}$), estratta dalla mediana del cluster vincente HDBSCAN sull'embedding UMAP e processata con la correzione di $S_3$ calibrata (\cref{sec:calibrazione_s3}), confrontata con il modello di quarzo cristallino $\delta(\lambda) = 90^\circ \cdot \Delta n(\lambda)/\Delta n(\lambda_0) \cdot \lambda_0/\lambda$ (Sellmeier di Ghosh), riportato come riscontro indipendente. L'accordo è entro pochi gradi su tutti i canali. La lieve differenza rispetto ai valori di calibrazione (\cref{tab:s3_calibrazione}), più marcata al blu, riflette il bias di depolarizzazione della lettura combinata coseno-seno della ritardanza.}
    \label{tab:qw_ritardanza}
    \begin{tabular}{lcccc}
        \hline
        \textbf{Canale} & $\lambda_\mathrm{eff}$ [\si{\nano\meter}] & $\tilde\delta_\mathrm{win}$ [\si{\degree}] & $\delta_\mathrm{quarzo}$ [\si{\degree}] & Scarto [\si{\degree}] \\
        \hline
        Rosso & 626 & $87{,}1$  & $90{,}9$  & $-3{,}8$ \\
        Verde & 536 & $108{,}6$ & $107{,}5$ & $+1{,}1$ \\
        Blu   & 466 & $128{,}5$ & $126{,}9$ & $+1{,}6$ \\
        \hline
    \end{tabular}
\end{table}
...
Le mappe dell'orientazione dell'asse veloce $\theta$ e della ritardanza $\delta$ sono riportate per i tre canali colore in \cref{fig:hw_theta_delta}. L'asse veloce risulta uniforme entro il disco della lamina, su un valore prossimo a $37^\circ$; lo scarto rispetto all'orientazione meccanica nominale di \SI{50}{\degree} corrisponde all'offset fra il riferimento del supporto della lamina e l'asse di polarizzazione incidente del display, rispetto al quale la pipeline misura tutti gli angoli polarimetrici (\cref{sec:allineamento}). La ritardanza è anch'essa uniforme nel disco, ma il suo valore \emph{decresce} passando dal canale rosso al canale blu, in direzione opposta a quella attesa per una lamina zero-order, in cui $\delta \propto 1/\lambda$ cresce verso le lunghezze d'onda corte. L'algoritmo di estrazione discusso nella \cref{subsec:modello_retardance} restituisce $\delta$ nell'intervallo $[0^\circ, 360^\circ)$ tramite $\arctan_2(\sin\delta, \cos\delta)$, con il segno di $\sin\delta$ fissato univocamente dal parametro $S_3$; il valore ricostruito è pertanto privo di ambiguità e questa decrescita è una caratteristica genuina del dato, la cui interpretazione è sviluppata più avanti.
...
I tre valori misurati decrescono in modo monotono dal rosso al blu ($176{,}9 \to 156{,}8 \to 134{,}5^\circ$), in direzione opposta alla previsione a singolo ordine, che cresce ($182{,}0 \to 212{,}6 \to 244{,}5^\circ$). Per un ritardatore attraversato una sola volta la ritardanza $\delta = 2\pi\,\Delta n(\lambda)\,d/\lambda$ cresce \emph{sempre} verso le lunghezze d'onda corte, poiché vi concorre il fattore geometrico $1/\lambda$, a cui un'eventuale birifrangenza $\Delta n(\lambda)$ in regime di dispersione normale non fa che sommarsi. Una ritardanza misurata \emph{decrescente} è perciò incompatibile con qualunque ritardatore a singolo passaggio, indipendentemente dal materiale. Un fit empirico della forma $\delta = k/\lambda^{\,n}$ sui tre punti restituisce un esponente $n \approx -0{,}9$; la branch complementare $360^\circ - \delta$, ascendente, darebbe $n \approx +0{,}7$. In entrambi i casi $|n| < 1$, mentre il singolo passaggio impone $n \geq 1$ (il solo fattore geometrico fissa $n = 1$, la dispersione normale lo porta oltre): l'esponente non rappresenta una legge di dispersione fisica, e il suo stesso cambiare di segno al variare della convenzione di elicità conferma che si tratta di un artefatto.

Le possibili cause sono state vagliate una per una. Un errore sistematico della catena di analisi è escluso dalla validazione offerta dalla lamina $\lambda/4$ (\cref{subsec:risultati_qw}), che condivide con questa misura il modello di Stokes, la correzione cromatica di $S_3$ e le lunghezze d'onda effettive, e riproduce il proprio riferimento entro $\SI{4}{\degree}$ su tutti i canali, blu compreso. Una depolarizzazione differenziale del campione è esclusa dal grado di polarizzazione totale misurato all'interno dei due dischi, che risulta identico fra le due lamine a ciascun canale (circa $0{,}77$ al blu per entrambe) e dunque attribuibile al sistema di illuminazione e non allo spessore di materiale attraversato. Resta da considerare una dispersione del materiale così anomala da invertire l'andamento geometrico: per produrre una decrescita reale di $\delta$ verso il blu la birifrangenza dovrebbe diminuire più rapidamente di quanto il fattore $1/\lambda$ cresca, un comportamento estraneo a un ritardatore trasparente nel visibile e perciò scartato senza bisogno di ipotesi sul materiale specifico.

Resta da escludere che la decrescita sia un'inversione di segno, prodotta da una lamina inserita con assi rapido e lento scambiati: una tale configurazione manda $\delta$ in $360^\circ - \delta$ e riporterebbe i valori al ramo ascendente. L'ipotesi è smentita dal segno della componente circolare. Uno scambio fra asse veloce e asse lento non altera né l'asse veloce ricostruito né l'AoLP, che dipendono unicamente da $S_1$ e $S_2$ (\cref{sec:retardance}), e si manifesta esclusivamente nel segno di $S_3$. Il parametro $S_3$ misurato all'interno del disco è positivo su tutti i canali in cui è apprezzabilmente diverso da zero, con lo stesso segno della lamina $\lambda/4$ (\cref{subsec:risultati_qw}): le due lamine condividono dunque la medesima elicità, e il segno di $S_3$, fissato globalmente e validato dalla $\lambda/4$ zero-order, è quello corretto anche in questa misura. La decrescita è perciò genuina e non un'inversione.

L'unica interpretazione compatibile è che la lamina sia un ritardatore \emph{multi-ordine}, la cui ritardanza vera supera molti multipli di $360^\circ$. Poiché $\delta_\mathrm{vero}(\lambda)$ cresce sempre verso il blu, una decrescita del valore misurato implica necessariamente l'attraversamento di uno o più multipli di $360^\circ$ fra un canale e il successivo: il passo apparente di circa $-20^\circ$ per banda corrisponde a un incremento reale di circa $+340^\circ$, ossia quasi un giro intero di ritardanza fra canali RGB adiacenti. Con soli tre campioni spettrali, una fase che avanza di tanto fra un punto e il successivo viene sotto-campionata e appare procedere lentamente all'indietro, in stretta analogia con l'effetto stroboscopico per cui una ruota in rapida rotazione sembra girare al contrario in una ripresa a basso numero di fotogrammi. La decrescita osservata è la manifestazione di questo avvolgimento (aliasing spettrale), non di una dispersione anomala del materiale.

Il canale rosso, prossimo alla lunghezza d'onda di progetto, conferma una mezz'onda ($\delta \approx 177^\circ \approx 180^\circ$), coerente con un ordine intero; proprio in prossimità di $180^\circ$ la componente $S_3 \propto \sin\delta$ si annulla e la ritardanza è massimamente sensibile al rumore, cosicché lo scarto di pochi gradi da $180^\circ$ non è significativo. Determinare l'ordine esatto non è invece possibile dai soli tre punti spettrali RGB. La ritardanza vera, pari a molti multipli di $360^\circ$, dipende dallo spessore e dalla dispersione di birifrangenza $\Delta n(\lambda)$ del materiale, entrambi ignoti; al crescere dell'ordine il valore avvolto diventa estremamente sensibile a $\Delta n(\lambda)$, cosicché con un materiale non caratterizzato e soli tre campioni spettrali il problema è degenere e ammette un'intera famiglia di ordini compatibili. Si conclude pertanto che la lamina $\lambda/2$ è un ritardatore multi-ordine — risultato solido e indipendente dal materiale, fondato sull'impossibilità della decrescita per un ritardatore a singolo passaggio — mentre l'ordine specifico resta indeterminato con i dati disponibili.
...
La rotazione cresce in modulo al diminuire della lunghezza d'onda, da $\SI{3.88}{\degree}$ al rosso a $\SI{6.15}{\degree}$ al blu, manifestazione della dispersione rotatoria ottica. Il fit con il modello di Drude semplificato $\psi(\lambda) = k/\lambda^2$ sull'AoLP mediano (\cref{fig:zucchero_fit}) restituisce $k = -1{,}39\cdot10^{6}~\si{\degree\nano\meter\squared}$ con $R^2 = 0{,}93$: il modulo di $k$ descrive la dispersione rotatoria, il segno negativo la rotazione destrogira. Il rapporto fra i moduli agli estremi spettrali, $|\psi_B|/|\psi_R| = 6{,}15/3{,}88 = 1{,}59$, è inferiore al rapporto $(\lambda_R/\lambda_B)^2 = 1{,}81$ atteso dal modello $1/\lambda^2$ puro, indicazione che alle lunghezze d'onda di lavoro la dispersione del saccarosio si discosta dalla pura legge inversa quadratica, come noto in presenza della risonanza nell'ultravioletto.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.55\textwidth]{Images/generated/zucchero/aolp_lambda_fit.pdf}
    \caption{Fit di dispersione rotatoria $\psi(\lambda) = k/\lambda^2$ (modello di Drude semplificato) sull'AoLP mediano del cluster vincente. I punti colorati sono l'AoLP per canale alle rispettive $\lambda_\mathrm{eff}$, con segno negativo coerente con il carattere destrogiro del saccarosio; la curva tratteggiata è il fit non lineare ai minimi quadrati.}
...
Il principale compromesso è introdotto dal downsampling: con il fattore $f = 4$ adottato in questo lavoro la risoluzione di analisi resta dell'ordine di $10^5$ pixel, sufficiente per i campioni macroscopici considerati ma tale da rendere irrisolvibili strutture più piccole di qualche centinaio di micrometri. Per i campioni macroscopici analizzati questa limitazione non è stata rilevante, ma applicazioni su scala microscopica richiederebbero un'elaborazione a blocchi o su GPU.
...
Un quarto fattore, specifico della ritardanza, è l'allineamento geometrico delle lamine. L'inserimento manuale della lamina $\lambda/4$ analizzatore e delle lamine campione non garantisce un'incidenza esattamente normale, e un'inclinazione residua di pochi gradi altera il cammino ottico e la birifrangenza efficace, con una variazione frazionaria del ritardo che cresce quadraticamente con l'angolo. Per una lamina zero-order l'effetto è trascurabile, dell'ordine del decimo di grado per inclinazioni di alcuni gradi, e la calibrazione di $S_3$, fondata sulla $\lambda/4$ zero-order, ne è di fatto immune; ma poiché la variazione si applica al ritardo \emph{totale}, su una lamina multi-ordine essa si amplifica in proporzione all'ordine, fino a raggiungere alcuni gradi sul valore avvolto, e contribuisce allo scarto residuo osservato sui campioni ad alta ritardanza.
