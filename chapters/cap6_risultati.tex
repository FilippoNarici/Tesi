...
\begin{table}[H]
    \centering
    \caption{Ritardanza della lamina $\lambda/2$ estratta dalla mediana del cluster vincente HDBSCAN sull'embedding UMAP, confrontata con la previsione zero-order $\delta_\mathrm{teorico} = 180^\circ \cdot \Delta n(\lambda)/\Delta n(\lambda_0) \cdot \lambda_0/\lambda$ (modello di Sellmeier di Ghosh per l'$\alpha$-quarzo). La ritardanza misurata \emph{decresce} verso il blu, in direzione opposta alla previsione zero-order, che \emph{cresce}: la lamina non è riconducibile a un ritardatore a singolo ordine (vedi testo).}
    \label{tab:hw_ritardanza}
    \begin{tabular}{lcccc}
        \hline
        \textbf{Canale} & $\lambda_\mathrm{eff}$ [\si{\nano\meter}] & $\tilde\delta_\mathrm{win}$ [\si{\degree}] & $\delta_\mathrm{teorico}$ [\si{\degree}] & Scarto [\si{\degree}] \\
        \hline
        Rosso & 626 & $176{,}9$ & $181{,}8$ & $-4{,}9$ \\
        Verde & 536 & $157{,}2$ & $214{,}9$ & $-57{,}7$ \\
        Blu   & 466 & $135{,}9$ & $253{,}7$ & $-117{,}8$ \\
        \hline
    \end{tabular}
\end{table}
...
\begin{figure}[H]
    ...
    \caption{Fit di dispersione zero-order $\delta(\lambda) = k/\lambda$ sulla lamina $\lambda/2$, adottato come modello di riferimento. I tre punti colorati sono le stime $\tilde\delta_{\mathrm{win}}$ del cluster vincente HDBSCAN per ciascun canale alla rispettiva $\lambda_\mathrm{eff}$; il diamante nero è il punto di design $(\lambda_0 = \SI{633}{\nano\meter},\,\delta_0 = 180^\circ)$ dichiarato dal costruttore, riportato come riferimento e non incluso nel fit. I punti misurati \emph{decrescono} verso le lunghezze d'onda corte, mentre la legge zero-order cresce: la curva non li descrive, comportamento interpretato nel testo come avvolgimento di una lamina multi-ordine.}
    \label{fig:hw_delta_fit}
\end{figure}

I tre valori misurati decrescono in modo monotono dal rosso al blu ($176{,}9 \to 157{,}2 \to 135{,}9^\circ$), in direzione opposta alla previsione zero-order, che cresce ($181{,}8 \to 214{,}9 \to 253{,}7^\circ$). Per un ritardatore attraversato una sola volta la ritardanza $\delta = 2\pi\,\Delta n(\lambda)\,d/\lambda$ cresce \emph{sempre} verso le lunghezze d'onda corte, poiché vi concorrono sia il fattore geometrico $1/\lambda$ sia l'aumento della birifrangenza $\Delta n(\lambda)$ in regime di dispersione normale. Una ritardanza misurata \emph{decrescente} è perciò incompatibile con qualunque ritardatore a singolo passaggio, indipendentemente dal materiale. Un fit empirico della forma $\delta = k/\lambda^{\,n}$ sui tre punti restituisce un esponente $n \approx -0{,}9$; la branch complementare $360^\circ - \delta$, ascendente, darebbe $n \approx +0{,}7$. In entrambi i casi $|n| < 1$, mentre il singolo passaggio impone $n \geq 1$ (per il quarzo $n \approx 1{,}14$): l'esponente non rappresenta una legge di dispersione fisica, e il suo stesso cambiare di segno al variare della convenzione di elicità conferma che si tratta di un artefatto.
...
Il canale rosso, prossimo alla lunghezza d'onda di progetto, conferma una mezz'onda ($\delta \approx 177^\circ \approx 180^\circ$), coerente con un ordine intero; proprio in prossimità di $180^\circ$ la componente $S_3 \propto \sin\delta$ si annulla e la ritardanza è massimamente sensibile al rumore, cosicché lo scarto di pochi gradi da $180^\circ$ non è significativo. Determinare l'ordine esatto non è invece possibile dai soli tre punti spettrali RGB. La ritardanza vera, pari a molti multipli di $360^\circ$, dipende dallo spessore e dalla dispersione di birifrangenza del materiale, ignoto; al crescere dell'ordine il valore avvolto diventa estremamente sensibile a $\Delta n(\lambda)$, e una scansione sistematica degli ordini su un modello di quarzo non individua alcuna soluzione di spessore fisicamente plausibile, con gli unici accostamenti accettabili confinati a ordini elevatissimi (spessori dell'ordine del millimetro) tipici della degenerazione introdotta dal sotto-campionamento. Si conclude pertanto che la lamina $\lambda/2$ è un ritardatore multi-ordine — risultato solido e indipendente dal materiale, fondato sull'impossibilità della decrescita per un ritardatore a singolo passaggio — mentre l'ordine specifico resta indeterminato con i dati disponibili.
...
\begin{figure}[H]
    ...
    \caption{Identificazione del cluster vincente sul canale rosso: embedding UMAP colorato per AoLP (sinistra), mappa spaziale con il cluster della soluzione evidenziato (centro) e istogramma dell'AoLP (destra). La mediana del cluster vincente è $\mathrm{AoLP} = -3{,}88^\circ$ (rotazione destrogira), corrispondente a una rotazione ottica di modulo $\tilde\psi_{\mathrm{win}} = \SI{3.88}{\degree}$.}
...
\begin{figure}[H]
    ...
    \caption{Cluster vincente sul canale blu, con la stessa struttura a tre pannelli della \cref{fig:zucchero_winner_r}. L'AoLP mediano è $-6{,}15^\circ$, ossia una rotazione di modulo $\tilde\psi_{\mathrm{win}} = \SI{6.15}{\degree}$, la più ampia dei tre canali.}
...
\begin{table}[H]
    ...
    \begin{tabular}{lcccc}
        ...
        \hline
        Rosso & 626 & $3{,}88$ & $58{,}9$  & $0{,}51$ \\
        Verde & 536 & $4{,}92$ & $80{,}3$  & $0{,}47$ \\
        Blu   & 466 & $6{,}15$ & $106{,}2$ & $0{,}45$ \\
        \hline
    \end{tabular}
\end{table}
