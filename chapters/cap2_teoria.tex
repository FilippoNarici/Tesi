...
\end{equation}
dove le parentesi angolari indicano la media temporale e le fasi sono riferite alla convenzione di propagazione $E_i \propto \cos(kz - \omega t + \delta_i)$, con $\delta = \delta_y - \delta_x$. $S_0$ rappresenta l'intensità totale del fascio; $S_1$ quantifica la prevalenza della polarizzazione orizzontale ($S_1 > 0$) o verticale ($S_1 < 0$); $S_2$ la prevalenza della polarizzazione a $+45^\circ$ ($S_2 > 0$) o $-45^\circ$ ($S_2 < 0$); $S_3$ la prevalenza della polarizzazione circolare destrogira ($S_3 < 0$) o levogira ($S_3 > 0$). Per il segno della componente circolare si adotta la convenzione del manuale di laboratorio~\cite{manuale_polarizzazione}, riferita a un osservatore rivolto verso la sorgente: alla polarizzazione destrogira (rotazione del vettore campo in senso orario per tale osservatore) si associa $S_3 < 0$, alla levogira $S_3 > 0$. Con la convenzione di fase sopra indicata, uno sfasamento $\sin\delta > 0$ produce una rotazione antioraria del campo per tale osservatore, sicché la \cref{eq:stokes_campi} risulta coerente con questa assegnazione. La scelta è convenzionale e si discosta da parte della letteratura, che lega il segno positivo alla circolare destra; viene mantenuta in modo coerente in tutto il lavoro e si riflette nella relazione di misura adottata per $S_3$ (\cref{sec:determinazione-s3}). I parametri di Stokes sono quantità misurabili, proporzionali alle intensità registrate da combinazioni appropriate di polarizzatori e lamine:
\begin{equation}
\label{eq:stokes_intensita}
\Svec = \begin{pmatrix} I_H + I_V \\ I_H - I_V \\ I_{+45} - I_{-45} \\ I_{LCP} - I_{RCP} \end{pmatrix},
\end{equation}
...
Per qualsiasi stato di polarizzazione vale la disuguaglianza:
\begin{equation}
...
\end{equation}
e vale 1 per luce totalmente polarizzata, caso in cui la \cref{eq:stokes_disuguaglianza} è satura, e 0 per luce completamente non polarizzata.
...
\end{equation}
L'\cref{eq:malus_generalizzata} può essere derivata applicando la matrice di Mueller di un polarizzatore lineare ruotato di $\theta$ (\cref{eq:mueller_pol}) al vettore di Stokes incidente e leggendo la componente $S_0'$ del vettore uscente (il formalismo delle matrici di Mueller è introdotto nella \cref{sec:mueller}). Si osservi che l'intensità trasmessa dipende solo dai primi tre parametri di Stokes e non da $S_3$: un polarizzatore lineare è insensibile alla polarizzazione circolare.
...
\end{equation}
Per un polarizzatore con asse ruotato di un angolo $\theta$, la matrice si ottiene applicando la trasformazione di rotazione $\Mmat_\text{pol}(\theta) = \Mmat_R(-\theta)\,\Mmat_\text{pol}(0^\circ)\,\Mmat_R(\theta)$, caso particolare della composizione della \cref{eq:mueller_composizione}, dove $\Mmat_R(\theta)$ è la matrice di rotazione nello spazio di Stokes (\cref{eq:mueller_rotatore}).
...
\end{equation}
I casi di interesse principale sono la lamina \textbf{quarto d'onda} ($\delta = \pi/2$) e la lamina \textbf{mezzo d'onda} ($\delta = \pi$). Per una lamina con asse veloce ruotato di un angolo $\alpha$, si applica la stessa trasformazione di rotazione descritta per il polarizzatore. La matrice è riportata nella forma standard della letteratura citata, riferita alla convenzione che associa $S_3 > 0$ alla polarizzazione destrogira; il passaggio alla convenzione adottata in questo lavoro corrisponde alla sostituzione $S_3 \rightarrow -S_3$, sotto la quale gli elementi $\pm\sin\delta$ della matrice si scambiano di segno. La distinzione è di pura etichetta e non incide sulle formule operative della catena di misura: l'estrazione della ritardanza (\cref{ch:analisi}) parametrizza lo stato con asse veloce ripiegato in $[0^\circ, 90^\circ]$ e $\delta \in [0^\circ, 360^\circ)$, parametrizzazione che assorbe la scelta di segno, e il segno effettivo di $S_3$ prodotto dallo strumento è fissato sperimentalmente (\cref{sec:determinazione-s3}).
...
\end{equation}
dove $\lambda$ è la lunghezza d'onda nel vuoto. Le \textbf{lamine d'onda} sfruttano questo effetto: una lamina $\lambda/4$ ($\delta = \pi/2$) converte polarizzazione lineare a $45^\circ$ in circolare, una lamina $\lambda/2$ ($\delta = \pi$) ruota il piano di polarizzazione di un angolo doppio rispetto all'orientazione della lamina. Poiché $\delta$ dipende inversamente da $\lambda$, una lamina calibrata a una data lunghezza d'onda introduce ritardi diversi alle altre lunghezze d'onda, aspetto che avrà conseguenze dirette sulla misura di $S_3$ con il sensore a tre canali colore (\cref{ch:analisi}). Una lamina si dice \textbf{zero-order} quando lo spessore è scelto in modo che il ritardo alla lunghezza d'onda di progetto coincida con la sola frazione d'onda desiderata ($\delta < 2\pi$), e \textbf{multi-order} quando al ritardo nominale si sommano ulteriori multipli interi di $2\pi$; a parità di frazione nominale, il ritardo totale di una lamina multi-order varia con la lunghezza d'onda in modo tanto più ripido quanto più alto è l'ordine, circostanza che ne rende la risposta spettrale rapidamente avvolta modulo $2\pi$. Esistono infine ritardatori a largo spettro (acromatici), nei quali la dispersione della birifrangenza è scelta in modo da compensare il fattore geometrico $1/\lambda$: il ritardo di fase resta così pressoché costante nel visibile, al prezzo di un ritardo ottico $\Delta n\, d$ con dispersione inversa, decrescente verso le lunghezze d'onda corte.
...
\end{equation}
dove $C$ è la costante fotoelastica del materiale e $\sigma_1$, $\sigma_2$ sono le tensioni principali nel piano perpendicolare alla direzione di propagazione. La ritardanza del campione, per la \cref{eq:ritardanza}, diventa quindi:
\begin{equation}
