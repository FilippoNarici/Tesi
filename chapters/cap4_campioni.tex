...
Una soluzione satura di saccarosio ($\SI{8.4}{\gram}$ in una boccetta da $\SI{6.9}{\milli\liter}$, cammino ottico $l = \SI{13}{\milli\meter}$, concentrazione gravimetrica $c_\text{grav} \approx \SI{1.22}{\gram\per\milli\liter}$) consente di verificare due aspetti: la rotazione uniforme del piano di polarizzazione, osservabile come variazione omogenea dell'AoLP, e la legge di dispersione $\alpha_\text{rot} \propto 1/\lambda^2$ (modello di Drude), sfruttando i tre canali colore del sensore. Il saccarosio destrogiro ha rotazione specifica $[\alpha]_D = \SI{66.47}{\degree\per(\deci\meter\cdot\gram\per\milli\liter)}$ alla riga D del sodio.
