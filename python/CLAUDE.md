...
| `final_umap.py` | analisi | UMAP 2D di Stokes, default `feature_mode='no_delta'` (feature `(s1,s2,s3,DoLP)`, pure Poincaré + magnitude — δ in gradi domina euclidean) e coloraggio AoLP. Figura 3-pannelli (mappa AoLP \| UMAP scatter \| istogramma AoLP), cmap `viridis` con autoscala percentile 1-99. Helper: `run_dataset_rgb(dataset)` batch (3 PDF single-panel publication-style per canale, stessa path/stile di interactive export, cache npz riutilizzato), `run_interactive_dataset(dataset, ch)` apre finestra con selettore poligonale custom click-only (`_PolyClickSelector` — niente preview-line su mouse motion per evitare lag su scatter densi): click sinistro = vertice, click destro = chiudi poligono → pixel selezionati in rosso su mappa+istogramma, vline rossa al baricentro ψ sull'istogramma con etichetta `ψ̄ = ±X.XX° ± Y.YY` accanto, etichetta `N = ...` rossa al centroide del cluster sullo scatter UMAP; tasti 'esc' annulla sequenza vertici, 'r' reset overlay+selector, 'enter' export 3 PDF single-panel publication-style (figsize 3.35+0.7 + ratio immagine, dpi 300, font serif, coerente con `final_thesis_figure`) in `Images/generated/<dataset>/aolp_umap/<CH>/{aolp_map,umap_scatter,aolp_hist}.pdf` (overwrite a ogni invocazione, no timestamp), 'q' quit; istruzioni stampate in console all'apertura; cache npz `outputs/umap_<dataset>_<CH>_cache.npz` per evitare recompute. All'import lo script fa `os.chdir(python/)` automatico, permette di lanciare da PyCharm con working directory = root del repo. CLI: `python final_umap.py [interactive|batch|legacy] [dataset] [R|G|B]` (default `interactive zucchero R`); flag `--no-cache` per forzare ricalcolo. Pearson r di UMAP1/2 vs sin(2ψ),cos(2ψ) per AoLP ciclica. Legacy `color_by='delta'` e `feature_mode='full'` restano disponibili. |
